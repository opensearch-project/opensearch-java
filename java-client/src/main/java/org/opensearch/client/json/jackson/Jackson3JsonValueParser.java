...
class Jackson3JsonValueParser {
    ...
    public JsonValue parseValue(JsonParser parser) {
        switch (parser.currentToken()) {
            ...
            case VALUE_STRING:
                return Jackson3JsonValueParser.DefaultJsonProvider.INSTANCE.createValue(parser.getString());

            case VALUE_NUMBER_FLOAT:
            case VALUE_NUMBER_INT:
                switch (parser.getNumberType()) {
                    case INT:
                        return Jackson3JsonValueParser.DefaultJsonProvider.INSTANCE.createValue(parser.getIntValue());
                    case LONG:
                        return Jackson3JsonValueParser.DefaultJsonProvider.INSTANCE.createValue(parser.getLongValue());
                    case FLOAT:
                    case DOUBLE:
                        return Jackson3JsonValueParser.DefaultJsonProvider.INSTANCE.createValue(parser.getDoubleValue());
                    case BIG_DECIMAL:
                        return Jackson3JsonValueParser.DefaultJsonProvider.INSTANCE.createValue(parser.getDecimalValue());
                    case BIG_INTEGER:
                        return Jackson3JsonValueParser.DefaultJsonProvider.INSTANCE.createValue(parser.getBigIntegerValue());
                }
