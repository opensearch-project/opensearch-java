...
public abstract class AbstractSearchTemplateRequestIT
		extends
	...
	public void testTemplateSearchResults() throws Exception {
		...
		var searchResponse = sendTemplateRequest(index, "NoDocuments", false,
				false);
	...
	private SearchTemplateResponse<SimpleDoc> sendTemplateRequest(String index,
			String title, boolean suggs, boolean aggs) throws IOException {
		return javaClient().searchTemplate(
				request -> request
					.index(index)
					.id(TEST_SEARCH_TEMPLATE)
					.params(Map.of(
							"title", JsonData.of(title),
							"suggs", JsonData.of(suggs),
							"aggs", JsonData.of(aggs))),
				SimpleDoc.class);
	}

	private void createDocuments(String index) throws IOException {
		createIndex(index);
		javaClient().create(_1 -> _1.index(index).id("1")
				.document(createDoc("Document 1", "The text of document 1", 1))
				.refresh(Refresh.True));
		javaClient().create(_1 -> _1.index(index).id("2")
				.document(createDoc("Document 2", "The text of document 2", 1))
				.refresh(Refresh.True));
		javaClient().create(_1 -> _1.index(index).id("3")
				.document(createDoc("Document 3", "The text of document 3", 2))
				.refresh(Refresh.True));
		javaClient().create(_1 -> _1.index(index).id("4")
				.document(createDoc("Document 4", "The text of document 4", 2))
				.refresh(Refresh.True));
	}
	...
	private void createSearchTemplate() throws IOException {
		var templateReq = new PutScriptRequest.Builder()
				.id(TEST_SEARCH_TEMPLATE)
