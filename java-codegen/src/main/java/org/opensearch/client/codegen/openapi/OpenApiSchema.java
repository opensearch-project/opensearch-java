...
public class OpenApiSchema extends OpenApiRefObject<OpenApiSchema, Schema> implements OpenApiExtensions {
    ...
    public Optional<Type> getType() {
        return getXDataType()
                .or(() -> Optional.ofNullable(getInner().getType()))
                .or(() -> {
    ...
    public Optional<Format> getFormat() {
        return Optional.ofNullable(getInner().getFormat())
                .map(Format::from);
    }
    ...
    private Optional<List<OpenApiSchema>> composedOf(List<Schema> schemas) {
        return Optional.ofNullable(schemas)
                .map(l -> l
                        .stream()
                        .map(s -> new OpenApiSchema(getParent(), s))
                        .toList());
    }
    ...
    public Set<String> getRequired() {
        return Optional.ofNullable((List<String>) getInner().getRequired())
                .map(HashSet::new)
    ...
    public enum Type {
        ARRAY, BOOLEAN, INTEGER, NUMBER, OBJECT, STRING, TIME;
